...
(async()=>{
    const data = require('../results/ChangHua_vet_results.json')
    const results =[];
    for (const i of data){
        const googleMap =await getGoogleMapApi('彰化縣'+i['address']) ;
        i['googleMapApi'] = googleMap
        results.push(i);
    }
    for (const i of results){
        if(i['googleMapApi']===null){
            mapApi = await getGoogleMapApi(i['hospital']+'彰化縣'+i['address'])
...
//node Puppeteer/ChangHuaVet.js > results/WordPress/ChangHuaVet.json
