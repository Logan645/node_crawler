...
async function getGoogleMapApi(keyword){
    ...
    try{
        ...
        await page.waitForSelector('#QA0Szd > div > div > div.w6VYqd > div.bJzME.tTVLSc > div > div.e07Vkf.kA9KIf > div > div > div:nth-child(10) > div > div.LCF4w > span.JpCtJf > span.DkEaL')
        const region = await page.$eval('#QA0Szd > div > div > div.w6VYqd > div.bJzME.tTVLSc > div > div.e07Vkf.kA9KIf > div > div > div:nth-child(10) > div > div.LCF4w > span.JpCtJf > span.DkEaL', el=>el.textContent.split('區')[0].slice(6)+'區')
...
(async()=>{
    const rawData = require('../results/Tainan_vet_results.json')
    const results =[];
    for (const i of rawData){
        const data =await getGoogleMapApi(i['address']) ;
        i['googleMapApi'] = data['googleMapApi']
        if (i['region']==='地址錯誤'){
    ...
    for (const i of results){
        if(i['googleMapApi']===null){
            const data = await getGoogleMapApi(i['hospital']+i['address'])
            i['googleMapApi'] = data['googleMapApi']
            if (i['region']==='地址錯誤'){
                i['region'] = data['region']
            }
...
//node Puppeteer/TainanVet.js > results/WordPress/TainanVet.json
